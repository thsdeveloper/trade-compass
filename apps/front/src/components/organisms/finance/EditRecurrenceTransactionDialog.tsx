...
export type EditRecurrenceOption = 'only_this' | 'this_and_future' | 'all';

interface EditRecurrenceTransactionDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onConfirm: (option: EditRecurrenceOption) => void;
  transaction: TransactionWithDetails;
}

export function EditRecurrenceTransactionDialog({
  open,
  onOpenChange,
  onConfirm,
  transaction,
}: EditRecurrenceTransactionDialogProps) {
  const [selectedOption, setSelectedOption] = useState<EditRecurrenceOption>('only_this');
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border p-3 hover:bg-slate-50 transition-colors">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    As outras transacoes desta despesa fixa nao serao alteradas
                  </div>
                </div>
              </label>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-blue-200 p-3 hover:bg-blue-50 transition-colors">
                ...
                />
                <div>
                  <div className="text-sm font-medium text-blue-700">Todas as transacoes pendentes</div>
                  <div className="text-xs text-blue-500">
                    Edita todas as pendentes desta despesa fixa (pagas nao sao afetadas)
                  </div>
                </div>
              </label>
            </div>
